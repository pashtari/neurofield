...
| --- | --- | --- | --- |
| image | 37 s | 9 min (14 models) | 3.5 h (24 images) |
| occupancy | 19 s | 5 min (15 models) | 25 min (5 shapes) |
| nerf | 10 min | 2.5 h (15 models) | 20 h (8 scenes) |
...
```bash
# image: 24 images x 14 models, about 3.5 hours
scripts/hpc/train.sh --clusters=accelgor --time=5:00:00 image

# occupancy: 5 shapes x 15 models, about 25 minutes
scripts/hpc/train.sh --clusters=accelgor --time=1:00:00 occupancy

# nerf: 15 models per scene, about 2.5 hours per scene, one job per scene
...
All 14 models on the 24 Kodak images, written to `logs/image/<image>/<model>/`.
...
All 15 models on the 5 Stanford shapes, written to
...
All 15 models on the 8 Blender scenes, with 37,500 steps per model. This is by
...
models="RFF PE-MLP MFN SIREN Gauss WIRE FINER Instant-NGP TensoRF-CP TensoRF-VM
        GA-Planes FUTON-cosine FUTON-sinc FUTON-triangle FUTON-lanczos"
for scene in data/nerf/blender/*/; do
